(ns maacl.sci-extensions
    (:require [maacl.custom]
              [maacl.performance]
              [mathbox.sci]
              [sci.ctx-store]
              [sci.core :as sci]))
...
;; ## MathBox.cljs installation
;;
;; This first section installs all `MathBox.cljs` namespaces into the project:

(mathbox.sci/install!)

;; See [`MathBox.cljs` via
;; SCI](https://mathbox.mentat.org/#mathbox.cljs-via-sci) for more details.
...
(def custom-namespace
  (sci/copy-ns maacl.custom
               (sci/create-ns 'maacl.custom)))
...
;; `:as` form to a namespace: `(:require [maacl.custom :as custom])`
(def custom-aliases
  {'custom 'maacl.custom})
